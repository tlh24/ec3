...
N_INF    = 24               # iterative inference passes per batch
ETA_ILV  = 0.15            # SGD step size for ILV
WD_ILV   = 0.15             # manual L2 coefficient applied every ILV step
...
USE_AMORT = True    # warm-start ILV from convolutional amortisation network
...
class Lifter(nn.Module):
	...
	def train_step(self,
		...
		for u in range(self.n_inference):
			...
			# Manual SGD update with per-layer L2 weight decay.
			# Layer 0 gets no WD; layer N_ILV-1 gets full wd_ilv.
			# Shape [1, N_ILV, 1, 1] broadcasts over [B, N_ILV, N_TOK, d].
			with torch.no_grad():
					wd = torch.linspace(0.0, self.wd_ilv, self.n_ilv,
											device=ilv.device).view(1, self.n_ilv, 1, 1)
					ilv.data -= self.eta_ilv * (g + wd * ilv.data)
	...
	def load_checkpoint(self, path: str) -> Optional[int]:
		...
		Falls back to strict=False if the checkpoint is missing amort_net keys
		(e.g. a checkpoint saved with use_amort=False loaded into a model with
		use_amort=True).  Missing amort_net weights are left at their random
		initialisation; all other keys must match exactly.
		"""
		checkpoint = torch.load(path, map_location='cpu', weights_only=True)
		try:
			self.load_state_dict(checkpoint['model_state'])
		except RuntimeError as e:
			missing = [k for k in e.args[0].split('\n') if 'amort_net' in k]
			if missing:
				print(f"load_checkpoint: amort_net weights absent in checkpoint "
				      f"-- loading with strict=False (amort_net stays randomly initialised)")
...
def test_ilv_independence(model: 'Lifter', device: torch.device,
    ...
    with torch.no_grad():
        wd = torch.linspace(0.0, model.wd_ilv, model.n_ilv,
                            device=ilv.device).view(1, model.n_ilv, 1, 1)
        ilv.data -= model.eta_ilv * (g + wd * ilv.data)
    ...
    for step in range(model.n_inference):
        ...
        with torch.no_grad():
            wd = torch.linspace(0.0, model.wd_ilv, model.n_ilv,
                                device=ilv2.device).view(1, model.n_ilv, 1, 1)
            ilv2.data -= model.eta_ilv * (g_i + wd * ilv2.data)
